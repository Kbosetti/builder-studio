...
- **Vercel:** team CEA Marketing `team_ruPVbZ5by79KqyoOnsn9JUZ8`.
  ...
  - Landing page concepts: project `cea-mitchell-landing-pages` (`prj_EFLn0LOFuuQFlvyRbZr3ZOHIymdz`). Source in `landing-pages/`. Forms post cross-origin to the Design Dollars intake with `data-page` keys `lp-*`.
  - **Custom domain routing:** `mitchellhomesliving.com` is assigned to the quiz project. Its `vercel.json` rewrites `/calculator`, `/math`, `/land`, `/no-land`, `/dreamer`, `/dreamer-carolinas`, `/thank-you`, `/style.css` to the landing-pages project and `/design-dollars`, `/design-dollars-nhi` to the Design Dollars project. Adding a page elsewhere means adding a rewrite here. The Design Dollars intake's `ALLOWED_ORIGINS` lists the custom domain.
  - None of the projects is linked to git. Deploy with the CLI from the source folder; each folder has a `.vercel/project.json` (gitignored) pointing at the right project. If it is missing, write it by hand rather than running `vercel link`, which creates a new project named after the folder.
